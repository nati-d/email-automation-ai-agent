...
import { getDraftCount } from "@/lib/api/email";
import {
  useInboxEmails,
  useSentEmails,
  useStarredEmails,
...
const getSidebarItems = (
  ...
  { label: "Sent", icon: Send, count: sentCount, href: "/dashboard/sent" },
  { label: "Drafts", icon: FileText, count: draftCount, href: "/drafts" },
  { label: "Tasks", icon: Tag, count: 0, href: "/dashboard/tasks" },
];
...
export function AppSidebar() {
  ...
  const { data: categoriesQuery, isLoading: isCategoriesLoading } =
    useCategories(false);
  React.useEffect(() => {
    if (!user) return;
    setLoading(isCategoriesLoading);
    setCategories((categoriesQuery?.categories as unknown as Category[]) || []);
  }, [user, isCategoriesLoading, categoriesQuery]);

  // Load email counts via queries (approximate by current page size)
  const { data: inboxData } = useInboxEmails(50);
  const { data: sentData } = useSentEmails(50);
  const { data: starredData } = useStarredEmails(50);
  React.useEffect(() => {
    if (!user) return;
    try {
      setInboxCount(
        (inboxData?.total_count as any) || (inboxData?.emails?.length ?? 0)
      );
      setSentCount(
        (sentData?.total_count as any) || (sentData?.emails?.length ?? 0)
      );
      setStarredCount(
        (starredData?.total_count as any) || (starredData?.emails?.length ?? 0)
      );
    } catch (error) {
      setInboxCount(0);
      setSentCount(0);
  ...
  }, [user, refreshTrigger, inboxData, sentData, starredData]);
  ...
  const handleCategoryClick = (categoryName: string, e: React.MouseEvent) => {
    ...
    // show navigation loading until route completes
    try {
      (useApp() as any).setIsNavigating?.(true);
    } catch {}
    router.push("/dashboard/inbox"); // Navigate to inbox route
  };
  ...
  const handleEmailTypeClick = (emailType: string, e: React.MouseEvent) => {
    ...
    const target = emailType.toLowerCase();
    if (target === "sent") router.push("/dashboard/sent");
    else if (target === "starred") router.push("/dashboard/starred");
  ...
  const handleDeleteCategory = async (
    ...
    try {
      ...
      if (currentCategory === categoryName) setCurrentCategory(null);
    } catch (error) {
  ...
  const handleAddAccount = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/api/oauth/add-another-account/initiate",
        {
          headers: {
      ...
      if (response.ok && data.authorization_url) {
        ...
        alert(
          data.detail?.message ||
            data.message ||
            "Failed to get OAuth URL for adding another account"
        );
      }
    } catch (error: any) {
  ...
  return (
    <>
      ...
      >
          ...
          <SidebarGroup>
            <SidebarMenu>
              {getSidebarItems(
                ...
                sentCount,
                draftCount
              ).map(({ label, icon: Icon, count, active, href }) => (
                <SidebarMenuItem key={label}>
                  <SidebarMenuButton
                    asChild
                    isActive={
                      label.toLowerCase() === currentEmailType &&
                      !currentCategory
                    }
                    className={
                      label.toLowerCase() === currentEmailType &&
                      !currentCategory
                        ? "relative bg-[color:var(--sidebar-accent)]/30 border-l-4 border-[color:var(--primary)] text-[color:var(--primary)] font-semibold"
                        : "hover:bg-[color:var(--sidebar-accent)]/20"
                    }
                    style={
                      label.toLowerCase() === currentEmailType &&
                      !currentCategory
                      ...
                      onClick={
                        label === "Inbox"
                          ...
                          ? (e) => {
                              e.preventDefault();
                              console.log(
                                "Drafts link clicked, navigating to /drafts"
                              );
                              router.push("/drafts");
                            }
                      ...
                      <>
                        ...
                        <SidebarMenuItem>
                          <SidebarMenuButton
                            ...
                            className={
                              !currentCategory
                                ? "bg-[color:var(--sidebar-accent)]/20 text-[color:var(--primary)] font-medium"
                                : "hover:bg-[color:var(--sidebar-accent)]/10"
                            }
                            style={
                              !currentCategory
                                ? {
        ...
        <SidebarFooter
          className="p-2 border-t w-full min-w-0 max-w-full overflow-x-hidden"
