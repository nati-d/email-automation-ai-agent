...
import { useInboxEmails } from "@/lib/queries";
...
export default function InboxPage() {
  const { user } = useApp();
  const { data, isLoading, isError } = useInboxEmails(50);
  const router = useRouter();
  const { replyToEmail } = useComposeModal();

  const emails = useMemo(() => {
    const list: any[] = (data?.emails as any) || [];
    return list.filter((e) => {
      // Prefer backend-provided type if available
      if (e?.email_type) return String(e.email_type).toLowerCase() === "inbox";

      // Heuristics to exclude sent/outbound
      const md = (e && e.metadata) || {};
      if (md?.is_outbound === true || md?.direction === "outbound")
        return false;
      if (e?.status && String(e.status).toLowerCase() === "sent") return false;
      if (
        e?.account_owner &&
        e?.email_holder &&
        e.account_owner === e.email_holder
      )
        return false;
      if (
        user?.email &&
        e?.sender &&
        String(e.sender).toLowerCase() === String(user.email).toLowerCase()
      )
        return false;
      return true;
    });
